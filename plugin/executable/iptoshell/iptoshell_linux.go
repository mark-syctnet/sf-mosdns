...
func (p *iptoshellPlugin) addIPtoshell(r *dns.Msg) error {
	for i := range r.Answer {
		...
		case *dns.A:
			...
			cmd := exec.Command(p.args.SetName4, rr.A, strconv.Itoa(p.args.Mask4), prefix.String() )
                        err := cmd.Run()
			if err != nil {
		...
		case *dns.AAAA:
			...
			cmd := exec.Command(p.args.SetName6, rr.AAAA,  strconv.Itoa(p.args.Mask6), prefix.String())
                        err := cmd.Run()
			if err != nil {
